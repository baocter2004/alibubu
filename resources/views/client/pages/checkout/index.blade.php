...
@section('content')
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-foreground mb-6">{{ __('client.checkout.title') }}</h1>
    ...
    <form action="{{ route('checkout.store') }}" method="POST" class="grid lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 bg-card border border-border rounded-xl p-5 md:p-6">
            <h2 class="font-bold text-foreground mb-5">{{ __('client.checkout.shipping_info') }}</h2>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    <label for="email" class="block text-sm font-medium text-foreground mb-1.5">{{ __('client.checkout.email') }}</label>
                ...
                <div class="sm:col-span-2">
                    <label for="note" class="block text-sm font-medium text-foreground mb-1.5">{{ __('client.checkout.note') }}</label>
            ...
            <div class="border-t border-border mt-6 pt-5">
                <h2 class="font-bold text-foreground mb-3">{{ __('client.checkout.payment_method') }}</h2>
                <div class="flex items-center gap-3 px-4 py-3 bg-primary/5 border border-primary/30 rounded-lg">
                    ...
                    <div>
                        <p class="text-sm font-medium text-foreground">{{ __('client.checkout.cod') }}</p>
                        <p class="text-xs text-muted-foreground">{{ __('client.checkout.cod_description') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <aside class="bg-card border border-border rounded-xl p-5 lg:sticky lg:top-24">
            <h2 class="font-bold text-foreground mb-4">{{ __('client.checkout.summary', ['count' => $items->sum('quantity')]) }}</h2>
