...
@section('title', __('admin/attribute.title.trash'))

@section('content')
    <div class="w-full bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
            <div>
                <h1 class="text-xl md:text-2xl font-semibold text-gray-900">{{ __('admin/attribute.title.trash') }}</h1>
                <p class="text-sm text-gray-500 mt-0.5">{{ __('admin/attribute.subtitle.trash') }}</p>
            </div>
        ...
        <div class="w-full overflow-x-auto rounded-lg border border-gray-200">
            <table class="min-w-[680px] w-full table-fixed admin-table admin-table-sticky">
                <thead>
                    <tr class="text-xs font-semibold tracking-wide text-left uppercase bg-primary text-white">
                        ...
                        <th class="w-[34%] px-4 py-3">{{ __('admin/attribute.fields.name') }}</th>
                        <th class="w-[30%] px-4 py-3">{{ __('common.labels.deleted_at') }}</th>
                        <th class="w-[26%] text-center px-4 py-3">{{ __('common.labels.actions') }}</th>
                    </tr>
                </thead>
                        ...
                        <tr class="text-sm text-gray-700 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-3">
                                    ...
                                    @endcan
                                    @can('attributes.force_delete')
                                        <form action="{{ route('admin.attributes.force-destroy', $attribute->id) }}"
