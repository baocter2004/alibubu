    ...
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            @forelse ($data['values'] ?? [] as $value)
                <span
                    class="inline-flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg border {{ !empty($value['is_active']) ? 'border-primary/20 bg-primary-soft text-primary' : 'border-gray-200 bg-gray-50 text-gray-500' }}">
                    {{ $value['value'] }}
                </span>
            @empty
