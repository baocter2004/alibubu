...
<div class="spec-item flex flex-col sm:flex-row gap-3 bg-gray-50 border border-gray-200 rounded-lg p-3" data-index="{{ $idx }}">
    ...
    <input type="text" name="specifications[{{ $idx }}][group]" value="{{ $spec['group'] ?? '' }}"
        ...
        class="sm:w-40 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">

    <div class="flex-1">
        <input type="text" name="specifications[{{ $idx }}][name]" value="{{ $spec['name'] ?? '' }}"
            placeholder="{{ __('admin/product.spec.name_placeholder') }}"
            class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has("specifications.$idx.name") ? 'is-invalid' : 'border-gray-300' }}">
    ...
    <div class="flex-1">
        <input type="text" name="specifications[{{ $idx }}][value]" value="{{ $spec['value'] ?? '' }}"
            ...
            class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has("specifications.$idx.value") ? 'is-invalid' : 'border-gray-300' }}">
