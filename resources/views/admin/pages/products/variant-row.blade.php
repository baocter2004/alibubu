...
<div class="variant-item bg-gray-50 border border-gray-200 rounded-lg p-4 relative" data-index="{{ $idx }}">
    <div class="flex items-center justify-between mb-4">
        <p class="text-sm font-semibold text-gray-700">
            <i class="fa-solid fa-layer-group text-blue-500 mr-1.5"></i>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
            <label class="block text-sm font-medium text-blue-500 mb-2">{{ __('admin/product.fields.sku') }}</label>
            <input type="text" name="variants[{{ $idx }}][sku]" value="{{ $variant['sku'] ?? '' }}"
                class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has("variants.$idx.sku") ? 'is-invalid' : 'border-gray-300' }}">
        ...
        <div>
            ...
            <input type="number" name="variants[{{ $idx }}][price]" value="{{ $variant['price'] ?? '' }}" min="0"
                step="1000"
                class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has("variants.$idx.price") ? 'is-invalid' : 'border-gray-300' }}">
        ...
        <div>
            <label
                class="block text-sm font-medium text-blue-500 mb-2">{{ __('admin/product.fields.sale_price') }}</label>
            <input type="number" name="variants[{{ $idx }}][sale_price]" value="{{ $variant['sale_price'] ?? '' }}"
                min="0" step="1000"
                class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has("variants.$idx.sale_price") ? 'is-invalid' : 'border-gray-300' }}">
    ...
    <label class="flex items-center gap-2 mt-4 cursor-pointer">
        ...
        <input type="checkbox" name="variants[{{ $idx }}][is_active]" value="1"
            @checked($variant['is_active'] ?? true) class="h-4 w-4 rounded accent-blue-500">
